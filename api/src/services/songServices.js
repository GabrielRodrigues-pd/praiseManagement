...
6. Mutação corrigida: criar clone com spread antes de modificar
7. Geração automática de ID em addSong: id: Date.now()
8. Verificação de exclusão em deleteSong: Confere se algo foi realmente removido
9. Service agora gerencia leitura/escrita internamente (controller não chama getAllSongs)
...
export function addSong(newSongs) {
  if (!newSongs || typeof newSongs !== "object") {
    throw new Error("Invalid song data");
  }
  const songs = [...getAllSongs(), { ...newSongs, id: Date.now() }];
  saveSongs(songs);
  return songs;
}
...
export function updateSong(updatedSong, id) {
